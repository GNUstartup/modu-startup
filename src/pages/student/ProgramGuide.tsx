...
// Design Components
const DefinitionBox = ({ title, children, color = "indigo" }: { title: string, children: React.ReactNode, color?: "indigo" | "blue" | "green" | "purple" | "orange" }) => {
    const colorMap = {
    ...
    return (
        <div className={`p-4 rounded-xl border font-medium mb-5 shadow-sm break-keep ${colorMap[color]}`}>
            <strong className="block mb-1 text-lg">{title}</strong>
            {children}
        </div>
    );
};
...
const NoticeCard = ({ children, color = "neutral" }: { children: React.ReactNode, color?: "neutral" | "red" | "blue" | "green" | "purple" | "orange" }) => {
    ...
    return (
        <div className={`p-5 mb-5 rounded-xl border flex items-start gap-3 shadow-sm break-keep ${colorMap[color]}`}>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-5">
            {steps.map((step, index) => (
                <div key={index} className="flex items-center mt-2">
                    <span className="font-bold text-sm text-neutral-800 bg-white px-4 py-2 rounded-full shadow border border-neutral-200 break-keep">
                        {step}
                    </span>
...
const DocumentTable = ({ headers, rows, color = "indigo" }: { headers: string[], rows: (string | React.ReactNode)[][], color?: string }) => {
    ...
    return (
        <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-sm mb-5 break-keep">
            <table className="min-w-full text-sm sm:text-base border-collapse text-left">
                <thead className={`${headerClass} text-white`}>
                    <tr>
                        {headers.map((h, i) => (
                            <th key={i} scope="col" className={`px-4 py-3 font-semibold ${i === 0 ? 'w-16 sm:w-24 text-center border-r border-white/20' : ''}`}>
                                {h}
                            </th>
                        ))}
                    </tr>
                ...
                <tbody className="text-neutral-800 divide-y divide-neutral-100">
                    {rows.map((row, rIdx) => (
                        <tr key={rIdx} className="hover:bg-neutral-50/50 transition-colors">
                            {row.map((cell, cIdx) => (
                                <td key={cIdx} className={`px-4 py-3 font-medium ${cIdx === 0 ? 'text-center text-neutral-500 border-r border-neutral-100 font-bold' : ''}`}>
                                    {cell}
                                </td>
                            ))}
                        </tr>
                ...
                <div className="bg-white rounded-2xl shadow-xl w-[90vw] md:w-[80vw] max-w-none max-h-[90vh] overflow-y-auto transform transition-all animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                    <div className="sticky top-0 bg-white border-b border-neutral-200 p-5 sm:p-6 flex justify-between items-start sm:items-center z-10 shadow-sm">
                        <div className="flex flex-col gap-1.5">
                            <h2 className={`text-xl sm:text-2xl font-bold break-keep text-${color}-800`}>{title}</h2>
                        </div>
                    ...
                    <div className="p-5 sm:p-8 text-neutral-800 text-sm sm:text-base leading-relaxed break-keep">
                        {children}
                    </div>
        ...
        <div className="min-h-screen bg-neutral-50 py-8 px-4 sm:px-6 lg:px-8 font-sans break-keep">
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className="bg-white p-5 rounded-xl shadow-sm border border-neutral-200 text-center">
                    <h1 className="text-xl sm:text-2xl font-bold text-neutral-900 flex justify-center items-center break-keep">
                ...
                <div className="bg-white border text-sm sm:text-base border-neutral-200 rounded-xl p-6 sm:p-8 shadow-sm break-keep text-neutral-800 leading-relaxed">
                    <h2 className="text-xl font-bold text-neutral-900 mb-4 pb-2 border-b border-neutral-100 flex items-center">
                        <CheckCircle2 className="w-5 h-5 mr-2 text-indigo-600" /> 공통 안내사항
                    </h2>

                    <DefinitionBox title="개요" color="indigo">
                        여러분이 받게 되는 ‘성장 지원금’은 단순한 장학금이나 지원금이 아니라 정부(중기부) 예산으로 집행되는 ‘사업비’입니다. 모든 지출은 창업아이템의 개발·사업화 목적에 직접 연관되어야 하며, ‘사업단이 대신 결제(구매대행)’하는 방식으로만 집행됩니다. 양산 목적 물품·용역 구매 불가, 시제품 제작 및 시장조사 관련 사항만 집행 가능.
                    </DefinitionBox>

                    <NoticeCard color="neutral">
                        <strong className="block text-indigo-700">지원금 사용</strong>
                        <p>현금을 참가자에게 지급하는 것이 아닌 참가자의 요청을 받고 사업단에서 대리 결제하는 방식. 즉, 참가자가 ‘필요한 품목’을 제안하면, 사업단이 승인 후 직접 결제하는 구조(외상 거래 기본, 선결제 불가).</p>
                    </NoticeCard>

                    <div className="mb-5 p-5 bg-neutral-50 border border-neutral-200 rounded-xl shadow-sm">
                        <strong className="block text-neutral-900 text-lg mb-3">기본 진행 절차</strong>
                    ...
                    <NoticeCard color="red">
                        <strong className="block text-red-800">유의사항</strong>
                        <ul className="list-disc pl-5 space-y-1.5 text-red-900 font-medium pb-1 mt-2">
                            <li>회차당 사업비 사용 금액 최대한 크게 하십시오.</li>
                            ...
                            <li>개인 명의 결제 불가.</li>
                            <li>불명확한 지출은 추후 부적정 판정 가능성이 큽니다.</li>
                        </ul>
                    </NoticeCard>
                </div>
                ...
                <div>
                    <h3 className="text-lg font-bold text-neutral-900 mb-4 flex items-center justify-center break-keep">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <button onClick={() => setOpenModal('여비')} className="group flex flex-col items-center justify-center p-6 bg-white border border-[#0288D1]/30 rounded-xl shadow-sm hover:shadow-md hover:border-[#0288D1] hover:bg-[#E1F5FE]/30 transition-all duration-200">
                            ...
                            <span className="text-lg font-bold text-[#0277BD] break-keep">여비</span>
                        </button>
                        ...
                        <button onClick={() => setOpenModal('재료비')} className="group flex flex-col items-center justify-center p-6 bg-white border border-[#2E7D32]/30 rounded-xl shadow-sm hover:shadow-md hover:border-[#2E7D32] hover:bg-[#E8F5E9]/30 transition-all duration-200">
                            ...
                            <span className="text-lg font-bold text-[#1B5E20] break-keep">재료비</span>
                        </button>
                        ...
                        <button onClick={() => setOpenModal('외주용역비')} className="group flex flex-col items-center justify-center p-6 bg-white border border-[#7B1FA2]/30 rounded-xl shadow-sm hover:shadow-md hover:border-[#7B1FA2] hover:bg-[#F3E5F5]/30 transition-all duration-200">
                            ...
                            <span className="text-lg font-bold text-[#4A148C] break-keep">외주용역비</span>
                        </button>
                        ...
                        <button onClick={() => setOpenModal('지급수수료')} className="group flex flex-col items-center justify-center p-6 bg-white border border-[#F57F17]/30 rounded-xl shadow-sm hover:shadow-md hover:border-[#F57F17] hover:bg-[#FFF8E1]/30 transition-all duration-200">
                            ...
                            <span className="text-lg font-bold text-[#E65100] break-keep">지급수수료(멘토링)</span>
                        </button>
                ...
                <Modal category="여비" title="여비" color="blue">
                    <DefinitionBox title="정의" color="blue">
                        소재지 외 출장 등의 사유로 집행하는 비용.
                    </DefinitionBox>

                    <NoticeCard color="blue">
                        <strong className="block text-lg">유의사항</strong>
                    ...
                    <h3 className="text-lg font-bold text-neutral-900 mb-2">지출 필요 서류 목록</h3>
                ...
                <Modal category="재료비" title="재료비" color="green">
                    <DefinitionBox title="정의" color="green">
                        시제품 제작에 소모되는 재료 구매 비용.
                    </DefinitionBox>
                    ...
                    <h3 className="text-lg font-bold text-green-800 mb-2">[온라인 결제 시] 서류 목록</h3>
                    ...
                    <h3 className="text-lg font-bold text-green-800 mt-6 mb-2">[오프라인 결제 시] 서류 목록</h3>
                    <DocumentTable color="green" headers={["구분", "제출 서류"]} rows={[
                ...
                <Modal category="외주용역비" title="외주용역비" color="purple">
                    <DefinitionBox title="정의" color="purple">
                        창업 고도화 목적으로 일부 공정을 외부 업체에 의뢰하여 제작하는 비용.
                    </DefinitionBox>
                    ...
                    <h3 className="text-lg font-bold text-purple-800 mb-2">[계약 시] 13종 서류 목록</h3>
                    ...
                    <h3 className="text-lg font-bold text-purple-800 mt-6 mb-2">[대금 지급 시] 4종 서류 목록</h3>
                ...
                <Modal category="지급수수료" title="지급수수료 (멘토링)" color="orange">
                    <DefinitionBox title="정의" color="orange">
                        비즈니스 모델 고도화, 제품 개선, 행정 자문 등의 목적으로 외부 전문가 멘토링 진행 후 지급하는 비용.
                    </DefinitionBox>

                    <NoticeCard color="orange">
                        <strong className="block text-lg mb-2">멘토 자격 유의사항</strong>
                        <p className="mb-2">창업/법·행정 전문가 위촉 목적만 허용 (위 사항 미달 시 지급 불가).</p>
                        <ol className="list-decimal pl-5 space-y-1 font-medium bg-orange-50/50 p-2 rounded">
                            <li>박사학위 소지자</li>
                            <li>석사학위 + 5년 이상 경력</li>
                            <li>학사학위 + 7년 이상 경력</li>
                            <li>조교수 이상의 대학교수</li>
